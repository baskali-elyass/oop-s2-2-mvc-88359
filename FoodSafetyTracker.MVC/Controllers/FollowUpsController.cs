...
public class FollowUpsController(ApplicationDbContext context) : Controller
{
    ...
    public async Task<IActionResult> Create([Bind("InspectionId,DueDate,Status")] FollowUp followUp)
    {
        ...
        {
            Log.Warning("FollowUp DueDate {DueDate} is before InspectionDate {InspDate} for InspectionId {Id}",
        ...
        Log.Information("FollowUp created: {Id} for InspectionId {InspectionId}, DueDate: {DueDate}",
    ...
    public async Task<IActionResult> Close(int id)
    {
        ...
        {
            Log.Warning("Attempted to close already-closed FollowUp {Id}", id);
        ...
        await context.SaveChangesAsync();
        Log.Information("FollowUp closed: {Id} on {ClosedDate}", id, followUp.ClosedDate);
        return RedirectToAction(nameof(Index));
    }
}
